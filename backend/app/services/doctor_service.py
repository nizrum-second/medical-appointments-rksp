...
def get_my_schedule(target_date, db: Session, current_user: User):
...
def get_my_patients(db: Session, current_user: User):
    doctor = db.query(Doctor).filter(Doctor.user_id == current_user.id).first()
    if not doctor:
        raise HTTPException(status_code=404, detail="Doctor profile not found")
...
def complete_appointment(appointment_id: int, db: Session, current_user: User):
    doctor = db.query(Doctor).filter(Doctor.user_id == current_user.id).first()
    if not doctor:
        raise HTTPException(status_code=404, detail="Doctor profile not found")
...
def get_my_doctor_profile(db: Session, current_user: User):
    doctor = db.query(Doctor).filter(Doctor.user_id == current_user.id).first()
    if not doctor:
        raise HTTPException(status_code=404, detail="Doctor profile not found")
