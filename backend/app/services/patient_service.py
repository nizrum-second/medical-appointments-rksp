...
def get_doctors(
    specialization: Optional[str], db: Session, current_user: User
):
...
def get_free_slots(
    doctor_id: int, target_date, db: Session, current_user: User
):
...
def create_appointment(
    appointment_data: AppointmentCreate, db: Session, current_user: User
):
    slot = (
        db.query(TimeSlot)
        .filter(
...
def get_my_appointments(db: Session, current_user: User) -> List[dict]:
    appointments = (
        db.query(Appointment)
...
def cancel_appointment(appointment_id: int, db: Session, current_user: User):
    appointment = (
        db.query(Appointment)
        .filter(
...
def get_doctors_by_service(service_id: int, db: Session, current_user: User):
    service = (
        db.query(Service)
