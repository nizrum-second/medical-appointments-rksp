...
def get_all_doctors(db: Session, current_user: User) -> List[dict]:
...
def create_doctor(doctor_data, db: Session, current_user: User) -> Doctor:
...
def update_doctor(
    doctor_id: int, doctor_data, db: Session, current_user: User
) -> Doctor:
...
def delete_doctor(doctor_id: int, db: Session, current_user: User) -> dict:
    doctor = db.query(Doctor).filter(Doctor.id == doctor_id).first()
    if not doctor:
        raise HTTPException(status_code=404, detail="Doctor not found")
    ...
    while current_date <= end_date:
        ...
        if template:
            ...
            while slot_start < slot_end:
                slot_end_time = slot_start + timedelta(minutes=30)
...
def confirm_appointment(
    appointment_id: int, db: Session, current_user: User
) -> dict:
    appointment = (
...
def reschedule_appointment(
    appointment_id: int, new_time_slot_id: int, db: Session, current_user: User
) -> dict:
    appointment = (
        db.query(Appointment).filter(Appointment.id == appointment_id).first()
    )
...
def get_doctors_load(
    start_date, end_date, db: Session, current_user: User
) -> list:
    doctors = db.query(Doctor).filter(Doctor.is_active == True).all()
    result = []
...
def get_all_users(
    full_name: Optional[str], db: Session, current_user: User
) -> list:
    query = db.query(User)
    if full_name:
...
def get_doctors_by_service_admin(
    service_id: int, db: Session, current_user: User
) -> list:
...
def get_services_by_doctor(
    doctor_id: int, db: Session, current_user: User
) -> list:
    doctor = db.query(Doctor).filter(Doctor.id == doctor_id).first()
    if not doctor:
        raise HTTPException(status_code=404, detail="Doctor not found")
